 ...
 * @since 1.20.0
 */
public final class JbangHooks extends AbstractActivatable<JbangHooks> implements Domain, JbangHookProvider, MatrixAware {
    private static final long serialVersionUID = 9006865770917440116L;

    private final Map<String, NamedJbangHooks> groups = new LinkedHashMap<>();
    ...
    @JsonIgnore
    private final org.jreleaser.model.api.hooks.JbangHooks immutable = new org.jreleaser.model.api.hooks.JbangHooks() {
        private static final long serialVersionUID = -3363667701774591539L;

        private Map<String, ? extends org.jreleaser.model.api.hooks.NamedJbangHooks> groups;
        ...
        public String getVersion() {
            return JbangHooks.this.getVersion();
        }

        @Override
        public String getCondition() {
            return JbangHooks.this.getCondition();
        }

        @Override
        public Map<String, ? extends org.jreleaser.model.api.hooks.NamedJbangHooks> getGroups() {
            if (null == groups) {
                groups = JbangHooks.this.groups.values().stream()
                    .map(NamedJbangHooks::asImmutable)
                    .collect(toMap(org.jreleaser.model.api.hooks.NamedJbangHooks::getName, identity()));
            }
            return groups;
        }
        ...
        public List<? extends org.jreleaser.model.api.hooks.JbangHook> getBefore() {
            if (null == before) {
                before = JbangHooks.this.before.stream()
        ...
        public List<? extends org.jreleaser.model.api.hooks.JbangHook> getSuccess() {
            if (null == success) {
                success = JbangHooks.this.success.stream()
                    .map(JbangHook::asImmutable)
                    .collect(toList());
            }
            return success;
        ...
        public List<? extends org.jreleaser.model.api.hooks.JbangHook> getFailure() {
            if (null == failure) {
                failure = JbangHooks.this.failure.stream()
                    .map(JbangHook::asImmutable)
                    .collect(toList());
            }
            return failure;
        ...
        public Map<String, String> getEnvironment() {
            return unmodifiableMap(JbangHooks.this.getEnvironment());
        }

        @Override
        public boolean isApplyDefaultMatrix() {
            return JbangHooks.this.isApplyDefaultMatrix();
        }
        ...
        public Active getActive() {
            return JbangHooks.this.getActive();
        }

        @Override
        public boolean isEnabled() {
            return JbangHooks.this.isEnabled();
        }

        @Override
        public Map<String, Object> asMap(boolean full) {
            return Collections.unmodifiableMap(JbangHooks.this.asMap(full));
        }
    };
    ...
    public org.jreleaser.model.api.hooks.JbangHooks asImmutable() {
        return immutable;
    }

    @Override
    public void merge(JbangHooks source) {
        super.merge(source);
    ...
    public Map<String, NamedJbangHooks> getGroups() {
        return groups;
    }

    public void setGroups(Map<String, NamedJbangHooks> groups) {
        this.groups.clear();
        this.groups.putAll(groups);
    }

    public void addGroup(NamedJbangHooks hook) {
