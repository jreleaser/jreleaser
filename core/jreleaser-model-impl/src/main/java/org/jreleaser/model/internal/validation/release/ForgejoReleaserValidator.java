 ...
 * @since 1.18.0
 */
public final class ForgejoReleaserValidator {
    private ForgejoReleaserValidator() {
        // noop
    }

    public static boolean validateForgejo(JReleaserContext context, Mode mode, ForgejoReleaser service, Errors errors) {
        if (null == service) return false;
        context.getLogger().debug("release.forgejo");

        validateGitService(context, mode, service, errors);

        if (isBlank(service.getApiEndpoint())) {
            errors.configuration(RB.$("validation_must_not_be_blank", "forgejo.apiEndpoint"));
        }
        ...
        service.getPrerelease().setPattern(
            checkProperty(context,
                PRERELEASE_PATTERN,
                "release.forgejo.prerelease.pattern",
