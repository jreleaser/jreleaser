 ...
 */
@CompileStatic
class OpenCollectiveAnnouncerImpl extends AbstractAnnouncer implements OpenCollectiveAnnouncer {
    ...
    @Inject
    OpenCollectiveAnnouncerImpl(ObjectFactory objects) {
        super(objects)
        ...
        slug = objects.property(String).convention(Providers.<String> notDefined())
    ...
    org.jreleaser.model.internal.announce.OpenCollectiveAnnouncer toModel() {
        org.jreleaser.model.internal.announce.OpenCollectiveAnnouncer announcer = new org.jreleaser.model.internal.announce.OpenCollectiveAnnouncer()
        fillProperties(announcer)
        if (host.present) announcer.host = host.get()
        if (token.present) announcer.token = token.get()
        if (slug.present) announcer.slug = slug.get()
